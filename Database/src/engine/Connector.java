...
public class Connector implements Connect{
	...
	private BTreeSet_interface join_column(BTreeSet_interface bTreeSet1, BTreeSet_interface bTreeSet2) {
		...
		ArrayList<String> allColumn_bTreeSet2_delete_common=new ArrayList<String>();
		...
		la1:for(int i=0;i<allColumn_bTreeSet2.size();i++) {
			for(int j=0;j<column_common.size();j++) {
				if(allColumn_bTreeSet2.get(i).equals(column_common.get(j)))
					continue la1;
			}
			allColumn_bTreeSet2_delete_common.add(allColumn_bTreeSet2.get(i));
			bt.addColumn(allColumn_bTreeSet2.get(i),allColumn_bTreeSet2_source.get(i));
		}
		return bt;
	}
